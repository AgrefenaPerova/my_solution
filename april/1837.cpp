...
public:
    Graph()
    {
        ...
        for (int i = 0; i < size; ++i)
        {
            ...
            for (int j = 0; j < 3; ++j)
            {
                ...
                names.insert(str);          // Добавляем все возможные имена и set сортирует их
            }
        }
        int k = -1;
        for (auto a : names)
        ...
        graph = new vector<int>[k + 1]();     // Создаем массив векторов
        dis = new int[k + 1] {0};             // k+1 так как считаем с 1
        used = new bool[k + 1] {0};
        for (int i = 0; i < size; ++i)
        {
            for (auto& a : groups[i])      // Для каждого члена группы отмечаем в графе что они связаны
            {                              // в а и b хранятся имена
                for (auto& b : groups[i])
                    // Условие которые исключает путь в самого себя
                    // Превращаем имя в индекс и добавляем связь
    ...
    void bfs()                  //O(n+m)
    {
        ...
        while (!q.empty())
        {
            int temp = q.front();
            q.pop();
            for (auto a : graph[temp])  // Бежим по графу 
            {
                if (!used[a])
                {
                    q.push(a);
...
private:
    ...
    int* dis;
    bool* used;
    unordered_map<string, int> index; // key - имя из names, value - индекс в векторе graph и массиве dis и used
